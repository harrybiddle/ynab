...
class SparkasseHeidelberg(Bank):
    ...
    def login(self, driver):
        ...
        login_label = self.unique(
            driver.find_elements_by_xpath(x), failure_msg="Cannot locate login textbox"
        )
    ...
    def assert_there_is_only_one_account(self, driver):
        ...
        assert rows == 4, (
            "Too many rows in finance status table. Do you "
    ...
    def locate_csv_and_transform_to_ynab_format(self, dir):
        csv_file = self.unique(
            fileutils.wait_for_file(dir, ".CSV"),
...
class CsvCamtToYnabFormat(object):
    ...
    def convert_csv(self, input_stream, output_stream):
        ...
        for row in input_csv:
            ...
            # write values
            output_csv.writerow(
                [new_date_string, payee, category, memo, outflow_string, inflow_string]
            )
