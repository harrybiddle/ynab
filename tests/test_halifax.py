...
class TestFileInverter(unittest.TestCase):
    def test_single_entry_invert(self):
        halifax = Halifax({'username': None},
                          {'password': None, 'challenge': None})
...
class EndToEndTest(EndToEndTestBase, unittest.TestCase):
    ...
    def mock_chrome_driver(self, chrome_driver):
        ...
        responses = {
            CHALLENGE_DESCRIPTION_CLASS_NAME: name
        }
        self.mock_driver_method(chrome_driver, 'find_element_by_class_name',
                                responses)
